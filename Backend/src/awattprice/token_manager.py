...
class APNs_Token_Manager:
    ...
    def write_database(self):
        ...
        if self.is_new_token:
            ...
            with self.db_manager.db:
                cursor.execute(
                    ...
                    (
                        ...
                        self.token_data["region_identifier"],
                        self.token_data["vat_selection"],
                        encoded_config,
                    ),
                )
            ...
            with self.db_manager.db:
                cursor.execute(
                    """ UPDATE token_storage SET region_identifier = ?, vat_selection = ?, configuration = ? WHERE token = ?""",
                    (
                        self.token_data["region_identifier"],
                        self.token_data["vat_selection"],
                        encoded_config,
    ...
    def set_data_task(self):
        ...
        if len(items) == 0:
            ...
            if (
                ...
                log.warning(
                    "A client resent his APNs token and configuration. "
